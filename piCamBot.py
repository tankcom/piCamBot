    ...
    def watchPIR(self):
        ...
        while True:
            ...
            if self.isPictureMoved and isNotEmpty2 and not self.ffmpegrunning: #only execute if pictures have been moved and the input folder is not empty and ffmpeg is not currently running
                ...
                args = ['bash', '-c', "ffmpeg -f concat -safe 0 -r 10 -i <(ls -d -1 /tmp/piCamBot/video/tmp/*.jpg | sed 's/^/file /') -vf format=yuv420p -c copy -b:v 800k /tmp/piCamBot/video/tmp4/a2.mp4; rm /tmp/piCamBot/video/tmp/*.jpg"]
                try:
